...
function printTableBody($limit)
{
  ...
  $query = sprintf("SELECT t1.id, t2.title, COUNT(t1.quest_abandon_time) AS abandoned_times, COUNT(t1.quest_complete_time) AS completed_times, MAX(t1.quest_abandon_time) AS last_abandoned, MAX(t1.quest_complete_time) AS last_completed " .
